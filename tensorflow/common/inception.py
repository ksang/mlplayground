...
class Inception:
	...
	def print_scores(self, pred, k=10, only_first_name=True, jsonFormat=True):
		"""
		...
		if jsonFormat:
			res = []
			for idx, cls in enumerate(reversed(top_k)):
				...
				ird = {'rank': idx, 'name': name, 'score': "{0:>6.2%}".format(score)}
				res.append(ird)
			print("RESULT_OUTPUT:")
